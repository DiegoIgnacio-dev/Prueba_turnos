...
Public Class N_InCO_TecMed


    'Declaraciones Generales
    Dim DD_Data As D_InCO_TecMed
    Sub New()
        DD_Data = New D_InCO_TecMed
    End Sub
    Function IRIS_WEBF_BUSCA_LIS_ADM_TECMED(ByVal DESDE As String, ByVal HASTA As String, ByVal ID_USUARIO As Integer) As List(Of E_InCo_TecMed)

        Dim List_in As New List(Of E_InCo_TecMed)
        List_in = DD_Data.IRIS_WEBF_BUSCA_LIS_ADM_TECMED(DESDE, HASTA, ID_USUARIO)

        Return List_in
    End Function
    Function Gen_Excel(ByVal MAIN_URL As String, ByVal DESDE As String, ByVal HASTA As String, ByVal ID_USUARIO As Integer, ByVal USU_NOM As String) As String
        'Declaraciones Generales
        Dim NN_Aten As New N_InCO_TecMed
        Dim Data_Aten As New List(Of E_InCo_TecMed)
        Dim Mx_Data(10, 0) As Object
        'Obtener parámetros para la consulta
        'Realizar Consulta
        Data_Aten = NN_Aten.IRIS_WEBF_BUSCA_LIS_ADM_TECMED(DESDE, HASTA, ID_USUARIO)
        ...
        For y = 0 To (Data_Aten.Count - 1)
            ...
            Mx_Data(1, y) = Data_Aten(y).USU_NOMBRE + " " + Data_Aten(y).USU_APELLIDO
        ...
        'nombrar hoja 
        sl.RenameWorksheet("Sheet1", "Ing vs Cos Usu")
        'titulo de la tabla
        sl.SetCellValue("B2", "Ingreso vs Costos por Usuario ")

        sl.SetCellValue("B3", "Usuario: " & USU_NOM)

        sl.SetCellValue("B4", "Desde: " & DESDE & " Hasta: " & DESDE)
        'nombre columnas
        sl.SetCellValue("A7", "Folio")
        sl.SetCellValue("B7", "Usuario")
        ...
        Dim Relative_Path As String = "IRISPDFDERIVADOS\" & USU_NOM & Format(Date.Now, "dd-MM-yyyy_hh-mm-ss") & ".xlsx"
