...
Public Class N_InCo_Sec

    'Declaraciones Generales
    Dim DD_Data As D_InCo_Sec
    Sub New()
        DD_Data = New D_InCo_Sec
    End Sub
    Function IRIS_WEBF_BUSCA_LIS_ADM_SECCION(ByVal DESDE As String, ByVal HASTA As String, ByVal ID_SECCION As Integer) As List(Of E_InCo_Sec)
        ...
        Dim List_in As New List(Of E_InCo_Sec)
        List_in = DD_Data.IRIS_WEBF_BUSCA_LIS_ADM_SECCION(DESDE, HASTA, ID_SECCION)

        Return List_in
    End Function
    Function Gen_Excel(ByVal MAIN_URL As String, ByVal DESDE As String, ByVal HASTA As String, ByVal ID_SECCION As Integer, ByVal SEC_DESC As String) As String
        'Declaraciones Generales
        Dim NN_Aten As New N_InCo_Sec
        Dim Data_Aten As New List(Of E_InCo_Sec)
        Dim Mx_Data(5, 0) As Object
        'Obtener parámetros para la consulta
        'Realizar Consulta
        Data_Aten = NN_Aten.IRIS_WEBF_BUSCA_LIS_ADM_SECCION(DESDE, HASTA, ID_SECCION)
        ...
        For y = 0 To (Data_Aten.Count - 1)
            ...
            Mx_Data(1, y) = Data_Aten(y).SECC_DESC
        ...
        'nombrar hoja 
        sl.RenameWorksheet("Sheet1", "Ing vs Cos Sec")
        'titulo de la tabla
        sl.SetCellValue("B2", "Ingreso vs Costos por Sección ")

        sl.SetCellValue("B3", "Sección: " & SEC_DESC)

        sl.SetCellValue("B4", "Desde: " & DESDE & " Hasta: " & DESDE)
        'nombre columnas
        sl.SetCellValue("A7", "Código")
        sl.SetCellValue("B7", "Sección")
        ...
        Dim Relative_Path As String = "IRISPDFDERIVADOS\" & SEC_DESC & Format(Date.Now, "dd-MM-yyyy_hh-mm-ss") & ".xlsx"
