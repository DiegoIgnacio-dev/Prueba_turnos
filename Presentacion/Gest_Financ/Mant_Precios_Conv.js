var MANT_PRECIOS_CONV;
(function (MANT_PRECIOS_CONV) {
    let fn_Init_Loading = {
        count: 0,
        max_count: 2,
        trigger: function () {
    ...
    let fn_Make_Table = () => {
        ...
        arrData.forEach((xItem, xIndex) => {
            objTable.addRow(xIndex, [
                (function () {
                    let strNow = `${xIndex + 1}`;
                    let strLng = `${arrData.length}`;
                ...
                xItem.CF_COD,
                xItem.CF_DESC,
                Chk_Enabled.toString(xIndex, null, (function () {
                ...
                `<input type="text" class="txt_table form-control" data-calc="pje1" value="${function () {
                    let raw = xItem.CF_PRECIO_AMB - xItem.CF_PRECIO_COSTO_T;
                    let strOut = numeral(TOOL.fn_cutDecimals(raw * xItem.CF_PRECIO_PJE_CONV * 0.01, 0, true)).format("0,0");
                    strOut = strOut.replace(/\./g, "d");
                    strOut = strOut.replace(/,/g, ".");
                    strOut = strOut.replace(/d/gi, ",");
                    return strOut;
                ...
                `<input type="text" class="txt_table form-control" data-array="CF_PRECIO_PJE_LAB" data-dec="0,0.00" value="${function () {
                    let raw = xItem.CF_PRECIO_PJE_LAB;
                    let strOut = numeral(raw).format("0,0.00");
                    strOut = strOut.replace(/\./g, "d");
                    strOut = strOut.replace(/,/g, ".");
                    strOut = strOut.replace(/d/gi, ",");
                ...
                `<input type="text" class="txt_table form-control" data-calc="pje2" value="${function () {
                    let raw = xItem.CF_PRECIO_AMB - xItem.CF_PRECIO_COSTO_T;
                    let strOut = numeral(TOOL.fn_cutDecimals(raw * xItem.CF_PRECIO_PJE_LAB * 0.01, 0, true)).format("0,0");
                    strOut = strOut.replace(/\./g, "d");
                    strOut = strOut.replace(/,/g, ".");
                    strOut = strOut.replace(/d/gi, ",");
                    return strOut;
    ...
    let fn_Txt_FocusOut = (Me) => {
        let arrField = $(Me.currentTarget).attr("data-array");
        let xVal = $(Me.currentTarget).val();
        let arrM;
        ...
        let floatVal = parseFloat(xVal);
        let strFormat = $(Me.currentTarget).attr("data-dec");
        let xRow = parseInt($(Me.currentTarget).parents("tr").attr("data-index"));
    ...
    let fn_Auto_Calc = (Me) => {
        let xIndex = parseInt($(Me.currentTarget).parents("tr").attr("data-index"));
        let txtDif = $(Me.currentTarget).parents("tr").find("input[data-calc=dif]");
        let txtPje1 = $(Me.currentTarget).parents("tr").find("input[data-calc=pje1]");
        let txtPje2 = $(Me.currentTarget).parents("tr").find("input[data-calc=pje2]");
        let fn_To_String = (input, format) => {
            let xOut = numeral(input).format(format);
            xOut = xOut.replace(/\./g, "d");
            xOut = xOut.replace(/,/g, ".");
            xOut = xOut.replace(/d/g, ",");
            return xOut;
        };
        let xTR = $(Me.currentTarget).parents("tr");
        switch ($(Me.currentTarget).attr("data-array")) {
            case "CF_PRECIO_AMB":
        ...
        let numDif = arrData[xIndex].CF_PRECIO_AMB - arrData[xIndex].CF_PRECIO_COSTO_T;
        let arrVal = [];
    ...
    let fn_Chk_Click = () => {
        Chk_Enabled.evClick = (Me) => {
            let xIndex = TOOL.fn_Base64().fromBase64($(Me.currentTarget).val());
    ...
    let countSave;
    //--Declaraciones AJAX---------------------------------------------------------------------------------------------
    let objAJAX_Year = new TOOL.class_AJAX("Mant_Precios_Conv.aspx/Select_Year", (resp) => {
        let arrSel = resp.d;
        Sel_Year.cleanAll();
    ...
    let objAJAX_Prev = new TOOL.class_AJAX("Mant_Precios_Conv.aspx/Select_Prev", (resp) => {
        let arrSel = resp.d;
        Sel_Prev.cleanAll();
    ...
    let objAJAX_Data_View = new TOOL.class_AJAX("Mant_Precios_Conv.aspx/Select_Data", (resp) => {
        arrData = resp.d;
        fn_Make_Table();
        Hide_Modal();
    }, (fail) => {
    ...
    let objAJAX_Data_Rec = new TOOL.class_AJAX("Mant_Precios_Conv.aspx/Write_Data", (resp) => {
    ...
    let objAJAX_Export = new TOOL.class_AJAX("Mant_Precios_Conv.aspx/Export", (resp) => {
        let xURL;
        xURL = resp.d;
        if (xURL != null) {
    ...
    let objAJAX_Copy = new TOOL.class_AJAX("Mant_Precios_Conv.aspx/Copy_Data", (resp) => {
        Hide_Modal();
    ...
    Btn_Confirm_Copy.click((Me) => {
        let strText = `¿Desea copiar los valores desde <strong>${Sel_Prev_From.getValue().text}</strong>, año <strong>${Sel_Year_From.getValue().text}</strong> `;
...
})(MANT_PRECIOS_CONV || (MANT_PRECIOS_CONV = {}));
//# sourceMappingURL=Mant_Precios_Conv.js.map