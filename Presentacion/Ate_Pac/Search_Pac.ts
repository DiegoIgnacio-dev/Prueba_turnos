namespace SEARCH_PAC {
    ...
    let arrPac: Array<iDataPac> = []
    let arrAte: Array<iDataAte> = []
    declare function Valid_RUT(strRUT: string): { Clean: string; Valid: boolean; Format: string; }
    ...
    ajaxDet.success = (resp) => {
        let arrDet: Array<iDataDet> = resp.d
    ...
    let eventKeyUp_01 = (me: JQueryEventObject) => {
        if (me.which == 13) {
            let strVal: string = $(me.currentTarget).val()
            strVal = strVal.trim()
    ...
    txtNAte.eventKeyUp = (me) => {
        let strOut: string = $(me.currentTarget).val()
