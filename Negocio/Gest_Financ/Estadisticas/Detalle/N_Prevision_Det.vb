...
Public Class N_Prevision_Det
    ...
    Function Gen_Excel(ByVal MAIN_URL As String, ByVal PREVE As Long, ByVal DATE1 As String, ByVal DATE2 As String) As String
        ...
        For y = 0 To (Data_Prev.Count - 1)
            ...
            End If
            Mx_Data(0, y) = y + 1
            Mx_Data(1, y) = Data_Prev(y).ID_ATENCION
            Mx_Data(2, y) = Data_Prev(y).PAC_NOMBRE + " " + Data_Prev(y).PAC_APELLIDO
            Mx_Data(3, y) = Data_Prev(y).PREVE_DESC
            Mx_Data(4, y) = Format(Data_Prev(y).ATE_FECHA, "dd/MM/yyyy")
            Mx_Data(5, y) = Data_Prev(y).PROC_DESC
            Mx_Data(6, y) = Data_Prev(y).CF_COD & ".-"
            Mx_Data(7, y) = Data_Prev(y).CF_DESC
            Mx_Data(8, y) = CInt(Data_Prev(y).ATE_DET_V_PREVI)
            Mx_Data(9, y) = CInt(Data_Prev(y).ATE_DET_V_PAGADO)
            Mx_Data(10, y) = CInt(Data_Prev(y).ATE_DET_V_COPAGO)
            Mx_Data(11, y) = CInt(Data_Prev(y).ATE_DET_V_PAGADO + Data_Prev(y).ATE_DET_V_COPAGO)
        Next y
        ...
        sl.SetCellValue("D7", "Previsión")
        sl.SetCellValue("E7", "Fecha")
        sl.SetCellValue("F7", "Lugar de TM")
        sl.SetCellValue("G7", "Código")
        sl.SetCellValue("H7", "Descripción")
        sl.SetCellValue("I7", "Valor sistema")
        sl.SetCellValue("J7", "Valor usuario")
        sl.SetCellValue("K7", "Valor copago")
        ...
        'insertar tabla
        tabla = sl.CreateTable("A7", CStr("L" & ltabla + 1))
