namespace H {
    ...
    export class Ts_Table {
        ...
        /**
         * Obtiene o establece si a tabla puede seleccionar alguno de sus ítems.
         */
        ...
        set isSelectable(value: boolean) {
            ...
            if (value == true) {
                this.obj_raw.find(`tbody tr`).click((Me: JQueryEventObject) => {
                    this.obj_raw.find(`tbody tr`).removeClass("selected")
         ...
         * @param alignment [opcional] Alineación, use "H.cHTMLAlign" para determinar dicha alineación.
         * @param width [opcional] Ancho en Píxeles.
         */
        public addHeader(text: string, alignment: iHTMLAlignElement, width: number = null): void {
         ...
         */
        public makeTable(callback: () => void = null): void {
            //Current Action
            let fn_do = () => {
                ...
                setTimeout(() => {
                    ...
                    this.obj_raw.children().fadeOut(240)
                    setTimeout(() => {
                        this.obj_raw.children().remove()
    ...
    export class Ts_Select extends Ts_Input {
        ...
        /**
         * Obtiene el texto del Select.
         */
        get text(): any {
            let xStr: string = this.obj_raw.children(`option:selected`).text()
...
namespace H {
    class Class_Form {
        ...
        /**
         * Mueve el Viewport al elemento indicado
         * @param id_dom ID del elemento al cual desplazar el viewport
         */
        public scrollToID(id_dom: string) {
            let elem: any = document.getElementById(id_dom)
...
namespace U {
    ...
    export class Ajax {
         ...
         * @param parameters 
         */
        public requestNow(parameters: any = null, callback: () => void = null) {
    ...
    /**
     * Funciones varias de conversión
     */
    export namespace CONV {
         ...
         */
        export let formatNum = (num: number, cantDec: number = null): string => {
            if (num == null) {
                return "-"
            }

            let arrRaw: Array<string> = `${parseFloat(`${num}`)}`.split(".")
            let strOut: string = ""
            //arrRaw[0] = enteros
            //arrRaw[1] = decimales

            let arrEnt: Array<string> = `${arrRaw[0]}`.split("")
            for (let i = arrEnt.length - 1; i >= 0; i--) {
                strOut = `${arrEnt[i]}${strOut}`

                let reverse: number = (arrEnt.length) - i
            ...
            if (cantDec >= arrRaw[1].length) {
                ...
                strOut = `${strOut},`
                let arrDec: Array<string> = arrRaw[1].split("")
