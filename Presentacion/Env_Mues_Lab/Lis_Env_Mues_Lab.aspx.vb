    ...
    Public Shared Function Excel(ByVal DOMAIN_URL As String, ByVal TIPO As Integer, ByVal DESDE As String, ByVal HASTA As String,
        ...
        If (data_det_ate.Count > 0) Then
            ...
            For y = 0 To (data_det_ate.Count - 1)
                ...
                Mx_Data(2, y) = Calcular_Edad(CDate(data_det_ate(y).PAC_FNAC)) & " " & "Años"
                Mx_Data(3, y) = Format(data_det_ate(y).ATE_FECHA, "dd/MM/yyyy")
                Mx_Data(4, y) = Format(data_det_ate(y).ATE_FECHA, "HH:mm:ss")
                Mx_Data(5, y) = data_det_ate(y).PROC_DESC

                ' End If

                Mx_Data(6, y) = data_det_ate(y).CF_DESC
