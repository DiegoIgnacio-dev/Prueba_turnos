...
Public Class N_DET_ATE_USU
    ...
    End Function
    Function IRIS_WEBF_BUSCA_LIS_ADM_RESU_FORMA_PAGO_LTM(ByVal DESDE As Date, ByVal HASTA As Date, ByVal ID_FP As Long, ByVal USU As Long) As List(Of E_IRIS_WEBF_BUSCA_EST_ESTADOS_INFORMAR45_6_RENDICION_PREVE)
        Return DD_Data.IRIS_WEBF_BUSCA_LIS_ADM_RESU_FORMA_PAGO_LTM(DESDE, HASTA, ID_FP, USU)
    End Function
    Function IRIS_WEBF_BUSCA_LIS_ADM_RESU_FORMA_PAGO_LTM_PREVE(ByVal DESDE As Date, ByVal HASTA As Date, ByVal ID_FP As Long, ByVal USU As Long) As List(Of E_IRIS_WEBF_BUSCA_EST_ESTADOS_INFORMAR45_6_RENDICION_PREVE)
        Return DD_Data.IRIS_WEBF_BUSCA_LIS_ADM_RESU_FORMA_PAGO_LTM_PREVE(DESDE, HASTA, ID_FP, USU)
    End Function
    ...
    Function Gen_Excel_usu_tp(ByVal MAIN_URL As String, ByVal DESDE As Date, ByVal HASTA As Date, ByVal ID_FP As Long, ByVal USU As Long) As String
        'Declaraciones Generales
        Dim NN_Date As New N_Date
        Dim NN_TM_Prevision As New N_DET_ATE_USU
        Dim Data_TM_Provision As New List(Of E_IRIS_WEBF_BUSCA_EST_ESTADOS_INFORMAR45_6_RENDICION_PREVE)
        ...
        Data_TM_Provision = NN_TM_Prevision.IRIS_WEBF_BUSCA_LIS_ADM_RESU_FORMA_PAGO_LTM(DESDE, HASTA, ID_FP, USU)
        ...
        For y = 0 To (Data_TM_Provision.Count - 1)
            ...
            End If
            Mx_Data(0, y) = y + 1
            Mx_Data(1, y) = Data_TM_Provision(y).TP_PAGO_DESC
            Mx_Data(2, y) = Data_TM_Provision(y).TOTAL_ATE
            Mx_Data(3, y) = Data_TM_Provision(y).TOT_FONASA
            Mx_Data(4, y) = Data_TM_Provision(y).TOTA_SIS
            Mx_Data(5, y) = Data_TM_Provision(y).TOTA_USU
            Mx_Data(6, y) = Data_TM_Provision(y).TOTA_COPA
            Mx_Data(7, y) = Data_TM_Provision(y).TOTA_USU + Data_TM_Provision(y).TOTA_COPA
        ...
        Dim grafico As SLChart
        'nombrar hoja 
        sl.RenameWorksheet("Sheet1", "Listado de Atencion por Usuario y T.Pago")
        'titulo de la tabla
        sl.SetCellValue("B2", "Listado de Atencion por Usuario")

        sl.SetCellValue("B3", "Usuario: " & Data_TM_Provision(0).USU_NIC)

        sl.SetCellValue("B4", "Desde: " & DESDE & " Hasta: " & HASTA)
        'nombre columnas
        sl.SetCellValue("A7", "#")
        sl.SetCellValue("B7", "Forma de Pago")
        sl.SetCellValue("C7", "Cantidad Atenciones")
        sl.SetCellValue("D7", "Cantidad Exámenes")
        ...
        Next i
        sl.SetCellValue("A" & ltabla + 1, "Totales:")
        ...
        Dim Ruta_save_local As String = System.Web.HttpContext.Current.Server.MapPath("~/")
        Dim Relative_Path As String = "IRISPDFDERIVADOS\" & Data_TM_Provision(0).USU_NIC & Format(Date.Now, "dd-MM-yyyy_hh-mm-ss") & ".xlsx"
        sl.SaveAs(Ruta_save_local & Relative_Path) 'Abrir el Archivo
        'Devolver la url del archivo generado
        Return MAIN_URL & "/" & Replace(Relative_Path, "\", "/")
    ...
    Function Gen_Excel_prev_tp(ByVal MAIN_URL As String, ByVal DESDE As Date, ByVal HASTA As Date, ByVal ID_FP As Long, ByVal USU As Long) As String
        'Declaraciones Generales
        Dim NN_Date As New N_Date
        Dim NN_TM_Prevision As New N_DET_ATE_USU
        Dim Data_TM_Provision As New List(Of E_IRIS_WEBF_BUSCA_EST_ESTADOS_INFORMAR45_6_RENDICION_PREVE)
        ...
        Data_TM_Provision = NN_TM_Prevision.IRIS_WEBF_BUSCA_LIS_ADM_RESU_FORMA_PAGO_LTM_PREVE(DESDE, HASTA, ID_FP, USU)
        ...
        Dim grafico As SLChart
        'nombrar hoja 
        sl.RenameWorksheet("Sheet1", "Listado de Atencion por Previsión y Usuario")
        'titulo de la tabla
        sl.SetCellValue("B2", "Listado de Atencion por Previsión y Usuario")

        sl.SetCellValue("B3", "Previsión: " & Data_TM_Provision(0).PREVE_DESC)
        sl.SetCellValue("B4", "Usuario: " & Data_TM_Provision(0).USU_NIC)

        sl.SetCellValue("B5", "Desde: " & DESDE & " Hasta: " & HASTA)
        'nombre columnas
        sl.SetCellValue("A7", "#")
        sl.SetCellValue("B7", "Forma de Pago")
        sl.SetCellValue("C7", "Cantidad Atenciones")
        ...
        Dim Ruta_save_local As String = System.Web.HttpContext.Current.Server.MapPath("~/")
        Dim Relative_Path As String = "IRISPDFDERIVADOS\" & Data_TM_Provision(0).PREVE_DESC & "_" & Data_TM_Provision(0).USU_NIC & Format(Date.Now, "dd-MM-yyyy_hh-mm-ss") & ".xlsx"
        sl.SaveAs(Ruta_save_local & Relative_Path) 'Abrir el Archivo
        'Devolver la url del archivo generado
        Return MAIN_URL & "/" & Replace(Relative_Path, "\", "/")
