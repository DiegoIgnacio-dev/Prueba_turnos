<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/Master.Master" CodeBehind="Prevision_Sum_Ger.aspx.vb" Inherits="Presentacion.Prevision_Sum_Ger" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="Cph_Head" runat="server">
    ...
    <script>
        ...
        function Ajax_Ddl() {
            ...
            $.ajax({
                "type": "POST",
                "url": "Prevision_Sum_Ger.aspx/Llenar_Ddl",
                //"data": '{}',
        ...
        function Ajax_DataTable() {
            ...
            $.ajax({
                "type": "POST",
                "url": "Prevision_Sum_Ger.aspx/Llenar_DataTable",
                "data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": function (response) {
        ...
        function Ajax_DataTable_TOTIS() {
            ...
            $.ajax({
                "type": "POST",
                "url": "Prevision_Sum_Ger.aspx/Llenar_DataTable_TOTIS",
                "data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": function (response) {
                    var json_receiver = response.d;
                    if (json_receiver.length > 0) {
                        Mx_Dtt_TOTIS = json_receiver;
                        $("#Div_Tabla_Data_TOTIS").empty();
                        Fill_DataTable();
                        Hide_Modal();
                        $("#Id_conte").show();
        ...
        function Ajax_Excel() {
            ...
            $.ajax({
                "type": "POST",
                "url": "Prevision_Sum_Ger.aspx/Gen_Excel",
                "data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": (resp) => {
    ...
    <script>
        ...
        function Ajax_ONCLICK(_PROG) {
            ...
            window.open(loc + "/Gest_Financ/Administracion/List_Ate_Prev_and_Prog_Ag_M_Ger.aspx?&P3=" + _PROG);
        }
        ...
        function Fill_DataTable() {
            ...
            for (i = 0; i < Mx_Dtt.length; i++) {
                //if (i = 0) {
                    ...
                    $("<tr>", {
                        "class": "manito"
                        //"onclick": `Ajax_ONCLICK("` + Mx_Dtt[i].TOTA_SIS + `",)`
                    }).append(
            ...
            $("<table>", {
                ...
                "width": "100%",
                "cellspacing": "0"
            }).appendTo("#Div_Tabla_Data_TOTIS");
            $("#DataTable_TOTIS").append(
                $("<thead>"),
                $("<tbody>")
            );
            $("#DataTable_TOTIS").attr("class", "table table-hover table-striped table-iris");
            $("#DataTable_TOTIS thead").attr("class", "cabezera");
            $("#DataTable_TOTIS thead").append(
                $("<tr>").append(
                    $("<th>", { "class": "text-center" }).text("#"),
                    $("<th>").text("Previsión"),
                    $("<th>", { "class": "text-center" }).text("Cant. Atenciones"),
            ...
            for (i = 0; i < Mx_Dtt_TOTIS.length; i++) {
                //if (i = 0) {
                $("#DataTable_TOTIS tbody").append(
                $("<tr>", {
                    "class": "manito"
                    //"onclick": `Ajax_ONCLICK("` + Mx_Dtt_TOTIS[i].TOTA_SIS + `",)`
                }).append(
                    $("<td>", { "align": "center" }).text(i + 1),
                    $("<td>").text(Mx_Dtt_TOTIS[i].PREVE_DESC),
                    $("<td>", { "align": "center" }).text(Mx_Dtt_TOTIS[i].TOTAL_ATE),
                    $("<td>", { "align": "center" }).text(Mx_Dtt_TOTIS[i].TOT_FONASA),
                    $("<td>", { "align": "center" }).text("$ " + cFormat.numToString(Mx_Dtt_TOTIS[i].TOTA_SIS, 0, ".", ",")),
                    $("<td>", { "align": "center" }).text("$ " + cFormat.numToString(Mx_Dtt_TOTIS[i].TOTA_USU, 0, ".", ",")),
                    $("<td>", { "align": "center" }).text("$ " + cFormat.numToString(Mx_Dtt_TOTIS[i].TOTA_COPA, 0, ".", ",")),
                    $("<td>", { "align": "center" }).text("$ " + cFormat.numToString(parseFloat(Mx_Dtt_TOTIS[i].TOTA_USU) + parseFloat(Mx_Dtt_TOTIS[i].TOTA_COPA), 0, ".", ","))
                )
            );
            }
    ...
    <div class="flex_col">
        <div class="card mb-3 border-bar">
            <div class="card-header bg-bar p-2">
                <h5>Resumen de Atenciones por Previsión-Procedencia</h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-lg">
        ...
        <div id="Id_conte">
            <div class="card mb-3 border-bar">
                <div class="card-header bg-bar p-2">
                    <h5>Valoralización de Atenciones por Previsión-Procedencia</h5>
                </div>
            ...
            <div class="card mb-3 border-bar">
                <div class="card-header bg-bar p-2">
                    <h5>Totales por Previsión</h5>
                </div>
                <div id="Div_Tabla_Data_TOTIS" class="card-body" style="overflow:auto">
                </div>
            </div>
