...
Public Class Recha_Mues
    ...
    <Services.WebMethod()>
    Public Shared Function Form_Table() As List(Of E_IRIS_WEBF_BUSCA_ETIQUETAS_RECEP_USUARIOS4_RECHAZO)
    ...
    <Services.WebMethod()>
    Public Shared Function Llenar_DataTable_Marcar(ByVal NUM_ATE As String, ByVal ID_ATE As Integer, ByVal MUESTRA() As Object, ByVal ID_TIPO As Integer, ByVal OBSER As String, ByVal TEXT_MOTIVO As String) As String
        ...
        Dim List_In() As Object
        Dim List_Out As New E_List_wDict22222
        Dim CB As String = ""
        Dim Pairs As New Dictionary(Of String, Long)


        For Each etiq In MUESTRA
            Dim strKey As String = etiq
        ...
        'For i = 0 To MUESTRA.Count - 1
        '    Dim strKey As String = MUESTRA(i)
        '    If i = 0 Then

        '        Pairs.Item(strKey) += 1

        '        cb_desc_comparador = MUESTRA(i)
        '    ElseIf ((i > 0) And (MUESTRA(i) <> cb_desc_comparador)) Then
        '        Pairs.Item(strKey) += 1
        ...
        List_Out.List_Data = MUESTRA
        List_Out.Dictionary = Pairs

        Dim numero As Integer
        Dim N_N As N_IRIS_OBTENER_INFO = New N_IRIS_OBTENER_INFO

        For Each AYYY In Pairs
            CB = AYYY.Key

            update_paciente3 = NN3.IRIS_WEBF_GRABA_ETIQUETA_RECEPCION_ETIQUETA_RECHAZO(ID_ATE, NUM_ATE, CB, ID_USER, ID_TIPO, OBSER)
            'data_paciente2 = NN2.IRIS_WEBF_BUSCA_ETIQUETA_BARRA_PACIENTE_RECEP_RECHAZO(NUM_ATE, CB)
            data_paciente2 = NN2.IRIS_WEBF_BUSCA_ETIQUETA_BARRA_PACIENTE_RECEP_RECHAZO_2(NUM_ATE, CB)        '<---------------------- EL NUEVO CON OMI y COD. TEST

            If data_paciente2.Count > 0 Then
                For ii = 0 To data_paciente2.Count - 1
                    '
                    update_paciente4 = NN4.IRIS_WEBF_UPDATE_ESTADO_RECEPCIONADO_RECEPCION_RECHAZO(data_paciente2(ii).ID_ATE_RES, ID_USER)
                    '4
                    update_paciente5 = NN5.IRIS_WEBF_UPDATE_ESTADO_RECEPCIONADO_RECEPCION_RECHAZO_INSERTA_RECHAZO(data_paciente2(ii).ID_ATE_RES, VALOR_TIPO_OBSERVACION)

                    If (data_paciente2(ii).ATE_NUM_OMI <> "" Or data_paciente2(ii).ATE_NUM_OMI <> "0") And data_paciente2(ii).ATE_CODIGO_TEST <> "" Then
                        numero = N_N.UPDATE_ESTADO_TEST_NULL(data_paciente2(ii).ATE_NUM_OMI, data_paciente2(ii).ATE_CODIGO_TEST)
                    End If
    ...
    Public Shared Function Ajax_Pendiente_Marcar(ByVal ATE_NUM As Integer, ByVal CB As String, ByVal ID_RECEP_ETI_RECHAZO_SUPREMO As Integer) As String
        ...
        data_paciente = NN_paciente.IRIS_WEBF_BUSCA_ETIQUETA_BARRA_PACIENTE_RECEP_RECHAZO_2(ATE_NUM, CB)
        ...
        If data_paciente.Count > 0 Then
            For i = 0 To data_paciente.Count - 1
                ...
                If (data_paciente(i).ATE_NUM_OMI <> "" Or data_paciente(i).ATE_NUM_OMI <> "0") And data_paciente(i).ATE_CODIGO_TEST <> "" Then
                    numero = N_N.UPDATE_ESTADO_TEST(data_paciente(i).ATE_NUM_OMI, data_paciente(i).ATE_CODIGO_TEST)
                End If
            Next i
        Else
            Return "null"
    ...
    Public Shared Function GUARDAR_AL_PISTOLEAR_EL_TUBO(ByVal NUM_ATE As String, ByVal ID_TIPO As Integer, ByVal OBSER As String, ByVal TEXT_MOTIVO As String) As String
        ...
        'data_paciente2 = NN2.IRIS_WEBF_BUSCA_ETIQUETA_BARRA_PACIENTE_RECEP_RECHAZO(NUM_ATE, CB)
        data_paciente2 = NN2.IRIS_WEBF_BUSCA_ETIQUETA_BARRA_PACIENTE_RECEP_RECHAZO_2(NUM_ATE, CB) '<--------- con OMIIIII
        Dim numero As Integer
        Dim N_N As N_IRIS_OBTENER_INFO = New N_IRIS_OBTENER_INFO
        If data_paciente2.Count > 0 Then
            For i = 0 To data_paciente2.Count - 1
                update_paciente4 = NN4.IRIS_WEBF_UPDATE_ESTADO_RECEPCIONADO_RECEPCION_RECHAZO(data_paciente2(0).ID_ATE_RES, ID_USER)
                update_paciente5 = NN5.IRIS_WEBF_UPDATE_ESTADO_RECEPCIONADO_RECEPCION_RECHAZO_INSERTA_RECHAZO(data_paciente2(0).ID_ATE_RES, VALOR_TIPO_OBSERVACION)

                If (data_paciente2(i).ATE_NUM_OMI <> "" Or data_paciente2(i).ATE_NUM_OMI <> "0") And data_paciente2(i).ATE_CODIGO_TEST <> "" Then
                    numero = N_N.UPDATE_ESTADO_TEST_NULL(data_paciente2(i).ATE_NUM_OMI, data_paciente2(i).ATE_CODIGO_TEST)
                End If
            Next i
            Return "tables"
        Else
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim C_P_ADMIN As HttpCookie = HttpContext.Current.Request.Cookies("P_ADMIN")
...
Public Class E_List_wDict22222
