    ...
    <script>
        ...
        function PRUEBA_ORDER_MED() {
            ...
            if (count == 0) {
                ...
                if (getParameterByName("IDA") != "") {
                    IDA = getParameterByName("IDA");
                } else {
                    IDA = Mx_DataTable.ID_ATENCION;
                }

                if (getParameterByName("IDU") != "") {
                    IDU = getParameterByName("IDU");
                } else {
                    IDU = Galletas.getGalleta("ID_USER");
                }
                ...
                $.ajax({
                    "type": "POST",
                    "url": "Ver_Orden.aspx/prueba_order_med",
                    "contentType": "application/json;  charset=utf-8",
                    "data": datos,
                    "dataType": "json",
        ...
        function Llenar_IMG() {
            ...
            let Data_Par = JSON.stringify({
                "ID_ATENCION": $("#txt_Folio").val()
            });
            //Debug
            AJAX_Ddl = $.ajax({
                "type": "POST",
                "url": "Ver_Orden.aspx/Get_Img",
                "data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": data => {
            ...
            Mx_IMG.forEach((imgx) => {
                if (imgx.IMG.length > 300) {
                    ...
                    if (imgx.FOTO_ATE_PLATAFORMA == 2) {
                        _Plat = "(PC)";
                    } else if (imgx.FOTO_ATE_PLATAFORMA == 1) {
                        _Plat = "(APP)";
                    ...
                    if (imgx.USU_NIC != null) {
                        Nombre_Doc = _Plat + " </br> " + moment(imgx.FECHA_ASOC).format("DD/MM/YYYY HH:mm:ss");
                    } else {
        ...
        function Show_Image(i) {
            ...
            if (Mx_IMG[i].FOTO_ATE_PLATAFORMA == 2) {
                console.log("uno");
                _Plat = "(PC)";
            } else if (Mx_IMG[i].FOTO_ATE_PLATAFORMA == 1) {
                console.log("dos");
                _Plat = "(APP)";
            } else {
                console.log("tres");
            ...
            if (Mx_IMG[i].USU_NIC != null) {
                Nombre_Doc_S = moment(Mx_IMG[i].FECHA_ASOC).format("DD/MM/YYYY HH:mm:ss") + " " + _Plat;
            } else {
        ...
        function Llenar_IMG_ASOC() {
            ...
            AJAX_Ddl = $.ajax({
                "type": "POST",
                "url": "Ver_Orden.aspx/Get_Img_Asoc",
                "data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": data => {
            ...
            if (Mx_IMG_Asoc != null) {
                ...
                Mx_IMG_Asoc.forEach((imgx) => {
                    if (imgx.IMG.length > 300) {
                        ...
                        ("<div class='col-lg'><img src='data:image/jpeg;base64," + imgx.IMG + "' class='mt-2' style='max-height:180px; width:auto' name='show_img_2' data-index='" + D_Index + "'/><div class='row text-center'><div class='col-lg'><label>Seleccionar<input type='checkbox' name='chk' value='" + D_Index + "' class='ml-2'/></label></div></div></div>")
                        );
                        if (count == 6) {
                            count = 0;
                            count_div += 1;
        ...
        function Show_Image_Asoc(i) {
            ...
            if (Mx_IMG_Asoc[i].FOTO_ATE_PLATAFORMA == 2) {
                _Plat = "(PC)";
            } else if (Mx_IMG_Asoc[i].FOTO_ATE_PLATAFORMA == 1) {
                _Plat = "(APP)";
            } else {
                _Plat = "(WEB)";
            }
            ...
            $("#Mdl_Image_Ate h3").text(() => {
                if (Mx_IMG_Asoc[i].USU_NIC != null) {
                    return moment(Mx_IMG_Asoc[i].FECHA_LOG).format("DD/MM/YYYY HH:mm:ss") + " " + _Plat;
                } else {
                    return "Web - " + moment(Mx_IMG_Asoc[i].FECHA_LOG).format("DD/MM/YYYY HH:mm:ss") + " " + _Plat;
                }
            });
            $("#Mdl_Image_Ate img").attr("src", "data:image/jpeg;base64," + Mx_IMG_Asoc[i].IMG);
                ...
                <div class="modal-body text-center">
                    <div class="row">
                        ...
                        <div class="col-lg-2 mt-1">
                            <button type="button" id="Btn_Buscar_Asoc" class="btn btn-buscar btn-block mt-0">Buscar</button>
                        </div>
                        <div class="col-lg-2 mt-1">
                            <button type="button" id="Btn_Asoc_Img" class="btn btn-primary btn-block mt-0">Vincular</button>
                        </div>
                    </div>
                    <div class="row">
