<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/Master.Master" CodeBehind="REP_LAB_EXA_2.aspx.vb" Inherits="Presentacion.REP_LAB_EXA_2" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="Cph_Head" runat="server">
    ...
    <script>
        ...
        function Ajax_Ddl_Proce() {
            ...
            AJAX_Ddl = $.ajax({
                "type": "POST",
                "url": "REP_LAB_EXA_2.aspx/Llenar_Ddl_LugarTM",
                //"data": Data_Par,
        ...
        function Ajax_Ddl() {
            ...
            $.ajax({
                "type": "POST",
                "url": "REP_LAB_EXA_2.aspx/Llenar_Ddl",
                //"data": '{}',
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
        ...
        function Ajax_DataTable() {
            ...
            $.ajax({
                "type": "POST",
                "url": "REP_LAB_EXA_2.aspx/Llenar_DataTable",
                "data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": function (response) {
        ...
        function Ajax_Excel() {
            ...
            $.ajax({
                "type": "POST",
                "url": "REP_LAB_EXA_2.aspx/Gen_Excel",
                "data": Data_Par,
                "contentType": "application/json;  charset=utf-8",
                "dataType": "json",
                "success": function (response) {
            ...
            for (i = 0; i < Mx_Dtt.length; i++) {
                ...
                $("#DataTable tbody").append(
                    $("<tr>").append(
                        ...
                        $("<td>").text(Mx_Dtt[i].ATE_AÑO),
                        $("<td>").text(Mx_Dtt[i].NACIONALIDAD),
                        $("<td>").text(Mx_Dtt[i].SEXO_DESC),
                        //ATE_FCHA
                        $("<td>", { "align": "center" }).text(function () {
                            var d = Mx_Dtt[i].ATE_FECHA;
                            var newd = moment.parseZone(d).format("DD-MM-YYYY hh:mm:ss");
                            return newd;
                        }),
                        $("<td>").text(Mx_Dtt[i].PREVISION),
                        $("<td>").text(Mx_Dtt[i].PROC_DESC),
                        $("<td>").text(Mx_Dtt[i].PROGRAMA),
                        $("<td>").text(Mx_Dtt[i].SUBPROGRAMA),
                        $("<td>").text(Mx_Dtt[i].CF_DESC),
                        $("<td>").text(Mx_Dtt[i].DETER),
                        $("<td>").text(Mx_Dtt[i].RESULTADO)
                    )
                );
            }
