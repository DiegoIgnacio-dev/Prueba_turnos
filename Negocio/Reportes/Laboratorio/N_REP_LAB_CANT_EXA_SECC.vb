...
Public Class N_REP_LAB_CANT_EXA_SECC
    ...
    End Function
    Function IRIS_WEBF_CMVM_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION_VALIDADOS_REVISADOS(ByVal ID_SECCION As Long, ByVal DESDE As Date, ByVal HASTA As Date, ByVal ID_PROC As Integer) As List(Of E_IRIS_WEBF_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION)
        Return DD_Data.IRIS_WEBF_CMVM_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION_VALIDADOS_REVISADOS(ID_SECCION, DESDE, HASTA, ID_PROC)
    End Function
    Function IRIS_WEBF_CMVM_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION_VALIDADOS(ByVal ID_SECCION As Long, ByVal DESDE As Date, ByVal HASTA As Date, ByVal ID_PROC As Integer) As List(Of E_IRIS_WEBF_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION)
        Return DD_Data.IRIS_WEBF_CMVM_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION_VALIDADOS(ID_SECCION, DESDE, HASTA, ID_PROC)
    End Function
    Function IRIS_WEBF_CMVM_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION_TODOS(ByVal ID_SECCION As Long, ByVal DESDE As Date, ByVal HASTA As Date, ByVal ID_PROC As Integer) As List(Of E_IRIS_WEBF_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION)
        Return DD_Data.IRIS_WEBF_CMVM_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION_TODOS(ID_SECCION, DESDE, HASTA, ID_PROC)
    End Function
    ...
    End Function
    Function Gen_Excel_Minds(ByVal MAIN_URL As String, ByVal ID_SECC As Long, ByVal DATE_str01 As String, ByVal DATE_str02 As String, ByVal VALOR As Integer) As String
        'Declaraciones Generales
        Dim NN_Date As New N_Date
        Dim NN_Exam As New N_REP_LAB_CANT_EXA_SECC
        Dim Data_JSON As New List(Of E_IRIS_WEBF_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION)
        ...
        If (VALOR = 0) Then
            Data_JSON = NN_Exam.IRIS_WEBF_CMVM_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION_VALIDADOS(ID_SECC, Date_01, Date_02, cookie_proc)
        ElseIf VALOR = 1 Then
            Data_JSON = NN_Exam.IRIS_WEBF_CMVM_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION_VALIDADOS_REVISADOS(ID_SECC, Date_01, Date_02, cookie_proc)
        ElseIf VALOR = 2 Then
            Data_JSON = NN_Exam.IRIS_WEBF_CMVM_BUSCA_LIS_ADM_RESU_CANT_EXAMENES_SECCION_TODOS(ID_SECC, Date_01, Date_02, cookie_proc)
        End If
        ...
        For y = 0 To (Data_JSON.Count - 1)
            ...
            Mx_Data(1, y) = Data_JSON(y).CF_DESC
            Mx_Data(2, y) = Data_JSON(y).TOTAL_ATE
            Mx_Data(3, y) = Data_JSON(y).TOTA_SIS
        Next y
        'Crear Tabla
        Dim Xls As New SLDocument
        Dim tablePosRow As Integer = 5
        ...
        Xls.MergeWorksheetCells(3, 1, 3, 3)
        'Llenar Cabeceras
        Xls.RenameWorksheet("Sheet1", "Atenciones por Sección: " & Mx_Data(1, 0))
        Dim tablePosCol_now As Integer = tablePosCol
        Xls.SetCellValue(tablePosRow, tablePosCol_now, "Código Fonasa") : tablePosCol_now += 1
        Xls.SetCellValue(tablePosRow, tablePosCol_now, "Descripción de Prestación") : tablePosCol_now += 1
        Xls.SetCellValue(tablePosRow, tablePosCol_now, "Cant. de Exám.") : tablePosCol_now += 1
        Xls.SetCellValue(tablePosRow, tablePosCol_now, "Venta")
        'Crear estilo para las cabeceras
        Dim colHeaderStyle = Xls.CreateStyle()
        colHeaderStyle.SetHorizontalAlignment(DocumentFormat.OpenXml.Spreadsheet.HorizontalAlignmentValues.CenterContinuous)
        colHeaderStyle.Font.Bold = True
