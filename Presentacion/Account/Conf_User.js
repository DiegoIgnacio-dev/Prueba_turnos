...
(function (CONF_USER) {
    ...
    (function (EV) {
        ...
        OBJ.Sel_Proc.eventChange((Me) => {
            let num_Prev = parseInt(`${OBJ.Sel_Prev.getValue().value}`);
            OBJ.objAJAX_Sel_Prev.requestNow({
                ID_PROC: OBJ.Sel_Proc.getValue().value
            }, () => {
                OBJ.Sel_Prev.setValue(num_Prev);
            });
        });
        OBJ.Sel_Prev.eventChange((Me) => {
            let num_Proc = parseInt(`${OBJ.Sel_Proc.getValue().value}`);
            OBJ.objAJAX_Sel_Proc.requestNow({
                ID_PREV: OBJ.Sel_Prev.getValue().value
            }, () => {
                OBJ.Sel_Proc.setValue(num_Proc);
            });
        });
