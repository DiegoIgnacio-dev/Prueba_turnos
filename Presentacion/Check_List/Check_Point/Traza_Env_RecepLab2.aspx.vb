...
Public Class Traza_Env_RecepLab2
    ...
    Public Shared Function Llenar_DataTable(ByVal TIPO As Integer,
        ...
        Dim CCC As New N_Date
        Dim date_01 As Date = CCC.strToDate(DESDE.Split("-")(2), DESDE.Split("-")(1), DESDE.Split("-")(0))
        Dim date_02 As Date = CCC.strToDate(HASTA.Split("-")(2), HASTA.Split("-")(1), HASTA.Split("-")(0))
        ...
        Try
            ...
            Data_Validado = NN.IRIS_WEBF_CMVM_BUSCA_ETIQUETAS_NO_RECEPCIONADA_Y_OTRAS_RVDR_2_2_SECCCION_POR_FOLIO_666_FECHAS_ENVIO_Y_RECEP_LAB_2_NUEVO_FIX_2(TIPO, date_01, date_02, ID_PRE, ID_ENVIO)
            If (Data_Validado.Count > 0) Then
                ...
                '            'Data_Validado(y).ID_USUARIO_RECEP = data_busca_estado_ate_trazabi_traza(0).ID_USUARIO_RECEP   <------- LOS ENVÍOS DE LABORATORIO
    ...
    Public Shared Function Excel(ByVal DOMAIN_URL As String,
        ...
        If (Data_Validado.Count > 0) Then
            ...
            '            'Data_Validado(y).ENVIO_FECHA_RECEP = data_busca_estado_ate_trazabi_traza(0).ENVIO_FECHA_RECEP
            '            'Data_Validado(y).ID_USUARIO_RECEP = data_busca_estado_ate_trazabi_traza(0).ID_USUARIO_RECEP <------ LOS ENVÍO DE LABORATORIO
            '            'Data_Validado(y).ID_ESTADO_RECEP = data_busca_estado_ate_trazabi_traza(0).ID_ESTADO_RECEP
            '            'Data_Validado(y).USUARIO_ENV_RECEP = data_busca_estado_ate_trazabi_traza(0).USUARIO_ENV_RECEP
            ...
            For y = 0 To (Data_Validado.Count - 1)
                ...
                If Data_Validado(y).USU_ENVIO = Nothing Then
                    Mx_Data(12, y) = ""
                Else
                    Mx_Data(12, y) = "SI"
                End If
                ...
                If Data_Validado(y).USU_ENVIO = Nothing Then
                    Mx_Data(13, y) = ""
                Else
                    Mx_Data(13, y) = Data_Validado(y).USU_ENVIO
                End If

                If Data_Validado(y).USU_ENVIO = Nothing Then
                    Mx_Data(14, y) = ""
                Else
                    Mx_Data(14, y) = Format(Data_Validado(y).ENVIO_ETI_FECHA, "dd/MM/yyyy HH:mm")
                End If

                If Data_Validado(y).USU_RECEP = Nothing Then
                    Mx_Data(15, y) = ""
                Else
                    Mx_Data(15, y) = "SI"
                End If
                ...
                If Data_Validado(y).USU_RECEP = Nothing Then
                    Mx_Data(16, y) = ""
                Else
                    Mx_Data(16, y) = Data_Validado(y).USU_RECEP
                End If

                If Data_Validado(y).USU_RECEP = Nothing Then
                    Mx_Data(17, y) = ""
                Else
                    Mx_Data(17, y) = Format(Data_Validado(y).RECEP_ETI_FECHA, "dd/MM/yyyy HH:mm")
                End If
        ...
        'sl.SetCellValue("C6", "Enviado SI:" & " " & rechSi & " / " & "NO:" & " " & rechNo)
        'sl.SetCellValue("E6", "TOTAL:" & " " & total)

        'nombrar hoja 
        sl.RenameWorksheet("Sheet1", " Trazabilidad por Envío y Recepción. ")

        'titulo de la tabla
        sl.SetCellValue("B2", " Trazabilidad por Envío y Recepción. ")
        ...
        sl.SetCellValue("P8", "Recepción")
        sl.SetCellValue("Q8", "Usuario Recep.")
        sl.SetCellValue("R8", "Fecha Recep.")
        ...
        'insertar tabla
        tabla = sl.CreateTable("A8", CStr("R" & ltabla + 1))
