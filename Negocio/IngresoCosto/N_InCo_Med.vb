...
Public Class N_InCo_Med


    'Declaraciones Generales
    Dim DD_Data As D_InCo_Med
    Sub New()
        DD_Data = New D_InCo_Med
    End Sub
    Function IRIS_WEBF_BUSCA_LIS_ADM_MED(ByVal DESDE As String, ByVal HASTA As String, ByVal ID_DOC As Integer) As List(Of E_InCo_Med)

        Dim List_in As New List(Of E_InCo_Med)
        List_in = DD_Data.IRIS_WEBF_BUSCA_LIS_ADM_MED(DESDE, HASTA, ID_DOC)

        Return List_in
    End Function
    Function Gen_Excel(ByVal MAIN_URL As String, ByVal DESDE As String, ByVal HASTA As String, ByVal ID_DOC As Integer, ByVal DOC_NOM As String) As String
        'Declaraciones Generales
        Dim NN_Aten As New N_InCo_Med
        Dim Data_Aten As New List(Of E_InCo_Med)
        Dim Mx_Data(10, 0) As Object
        'Obtener parámetros para la consulta
        'Realizar Consulta
        Data_Aten = NN_Aten.IRIS_WEBF_BUSCA_LIS_ADM_MED(DESDE, HASTA, ID_DOC)
        ...
        For y = 0 To (Data_Aten.Count - 1)
            ...
            Mx_Data(1, y) = Data_Aten(y).DOC_NOMBRE + " " + Data_Aten(y).DOC_APELLIDO
        ...
        'nombrar hoja 
        sl.RenameWorksheet("Sheet1", "Ing vs Cos Med")
        'titulo de la tabla
        sl.SetCellValue("B2", "Ingreso vs Costos por Médico ")

        sl.SetCellValue("B3", "Médico: " & DOC_NOM)

        sl.SetCellValue("B4", "Desde: " & DESDE & " Hasta: " & DESDE)
        'nombre columnas
        sl.SetCellValue("A7", "Folio")
        sl.SetCellValue("B7", "Médico")
        ...
        Dim Relative_Path As String = "IRISPDFDERIVADOS\" & DOC_NOM & Format(Date.Now, "dd-MM-yyyy_hh-mm-ss") & ".xlsx"
