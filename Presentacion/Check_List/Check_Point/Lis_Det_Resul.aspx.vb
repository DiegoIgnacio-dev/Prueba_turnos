    ...
    Public Shared Function Excel(ByVal DOMAIN_URL As String, ByVal TIPO As Integer, ByVal DESDE As String, ByVal HASTA As String,
        ...
        If (Data_Validado.Count > 0) Then
            ...
            For y = 0 To (Data_Validado.Count - 1)
                ...
                Mx_Data(3, y) = Data_Validado(y).ATE_DNI
                Mx_Data(4, y) = Data_Validado(y).NAC_DESC
                Mx_Data(5, y) = Data_Validado(y).PAC_NOMBRE + " " + Data_Validado(y).PAC_APELLIDO
                Mx_Data(6, y) = Format(Data_Validado(y).PAC_FNAC, "dd/MM/yyyy")
                Mx_Data(7, y) = Calcular_Edad(CDate(Data_Validado(y).PAC_FNAC)) & " " & "Años"
                Mx_Data(8, y) = Format(Data_Validado(y).ATE_FECHA, "dd/MM/yyyy")
                Mx_Data(9, y) = Format(Data_Validado(y).ATE_FECHA, "HH:mm:ss")
                Mx_Data(10, y) = Data_Validado(y).PROC_DESC

                If Data_Validado(y).ID_SEXO = "2" Then
                    Mx_Data(11, y) = "Femenino"
                Else
                    Mx_Data(11, y) = "Masculino"
                End If
                Mx_Data(12, y) = Data_Validado(y).PROGRA_DESC
                Mx_Data(13, y) = Data_Validado(y).SECTOR_DESC
                Mx_Data(14, y) = Data_Validado(y).CF_DESC
                Mx_Data(15, y) = Data_Validado(y).T_MUESTRA_DESC
                Mx_Data(16, y) = CInt(Data_Validado(y).CB_DESC)
                ...
                Else
                    Mx_Data(19, y) = "SI"
                End If
                Mx_Data(20, y) = Data_Validado(y).DOC_NOMBRE & " " & Data_Validado(y).DOC_APELLIDO
            Next y
        ...
        sl.SetCellValue("I8", "Fecha")
        sl.SetCellValue("J8", "Hora")
        sl.SetColumnWidth("J", 10)
        sl.SetCellValue("K8", "Lugar de TM")
        sl.SetColumnWidth("K", 25)
        sl.SetCellValue("L8", "Sexo")
        sl.SetCellValue("M8", "Programa")
        sl.SetCellValue("N8", "Sector")
        sl.SetCellValue("O8", "Examen")
        sl.SetColumnWidth("O", 30)
        sl.SetCellValue("P8", "Tipo Etiqueta")
        sl.SetCellValue("Q8", "CB")
        sl.SetColumnWidth("Q", 15)
        ...
        sl.SetCellValue("U8", "Médico")
        sl.SetColumnWidth("U", 30)
