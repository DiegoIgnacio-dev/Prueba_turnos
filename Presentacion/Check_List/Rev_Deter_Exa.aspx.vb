    ...
    Public Shared Function Excel(ByVal DOMAIN_URL As String, ByVal DESDE As String, ByVal HASTA As String,
        ...
        If (Data.Count > 0) Then
            ...
            For y = 0 To (Data.Count - 1)
                ...
                Mx_Data(5, y) = Format(Data(y).PAC_FNAC, "dd/MM/yyyy")
                Mx_Data(6, y) = Data(y).ATE_AÑO & " " & "Años"
                Mx_Data(7, y) = Data(y).PAC_RUT

                If (Data(y).ATE_DNI <> "") Then
                    Mx_Data(8, y) = Data(y).ATE_DNI
                Else
                    Mx_Data(8, y) = ""
                End If

                If (Data(y).NAC_DESC <> "") Then
                    Mx_Data(9, y) = Data(y).NAC_DESC
                Else
                    Mx_Data(9, y) = ""
                End If
                ...
                If (Data(y).SECTOR_DESC <> "") Then
                    Mx_Data(11, y) = Data(y).SECTOR_DESC
                Else
                    Mx_Data(11, y) = ""
                End If
                ...
                If (Data(y).DOC_NOMBRE = "" Or Data(y).DOC_APELLIDO = "") Then
        ...
        sl.SetCellValue("G8", "Edad")
        sl.SetColumnWidth("G", 10)
        sl.SetCellValue("H8", "Rut Paciente")
        sl.SetCellValue("I8", "DNI")
        sl.SetCellValue("J8", "Nacionalidad")
        sl.SetCellValue("K8", "Programa")
        sl.SetCellValue("L8", "Sector")
