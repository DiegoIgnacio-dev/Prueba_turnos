...
Public Class N_InCo_TP


    'Declaraciones Generales
    Dim DD_Data As D_InCo_TP
    Sub New()
        DD_Data = New D_InCo_TP
    End Sub
    Function IRIS_WEBF_BUSCA_LIS_ADM_ORD(ByVal DESDE As String, ByVal HASTA As String, ByVal ID_TP As Integer) As List(Of E_InCo_TP)

        Dim List_in As New List(Of E_InCo_TP)
        List_in = DD_Data.IRIS_WEBF_BUSCA_LIS_ADM_TP(DESDE, HASTA, ID_TP)

        Return List_in
    End Function
    Function Gen_Excel(ByVal MAIN_URL As String, ByVal DESDE As String, ByVal HASTA As String, ByVal ID_TP As Integer, ByVal TP_DESC As String) As String
        'Declaraciones Generales
        Dim NN_Aten As New N_InCo_TP
        Dim Data_Aten As New List(Of E_InCo_TP)
        Dim Mx_Data(10, 0) As Object
        'Obtener parámetros para la consulta
        'Realizar Consulta
        Data_Aten = NN_Aten.IRIS_WEBF_BUSCA_LIS_ADM_ORD(DESDE, HASTA, ID_TP)
        ...
        For y = 0 To (Data_Aten.Count - 1)
            ...
            Mx_Data(1, y) = Data_Aten(y).PREVE_DESC
            Mx_Data(2, y) = Format(Data_Aten(y).ATE_FECHA, "MM/dd/yyyy hh:mm:ss")
            Mx_Data(3, y) = Data_Aten(y).TP_PAGO_DESC
        ...
        'nombrar hoja 
        sl.RenameWorksheet("Sheet1", "Ing vs Cos TP")
        'titulo de la tabla
        sl.SetCellValue("B2", "Ingreso vs Costos por Forma de Pago")

        sl.SetCellValue("B3", "Forma Pago: " & TP_DESC)

        sl.SetCellValue("B4", "Desde: " & DESDE & " Hasta: " & DESDE)
        'nombre columnas
        sl.SetCellValue("A7", "Folio")
        sl.SetCellValue("B7", "Previsión")
        sl.SetCellValue("C7", "Fecha")
        sl.SetCellValue("D7", "Forma Pago")
        ...
        Dim Relative_Path As String = "IRISPDFDERIVADOS\" & TP_DESC & Format(Date.Now, "dd-MM-yyyy_hh-mm-ss") & ".xlsx"
